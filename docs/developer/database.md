...
=== "ภาษาไทย (Thai)"
    ...
    - **คอลัมน์**: `id` (UUID), `name` (String), `type` (Weapon, Consumable, etc.), `base_stats` (JSONB), `rarity` (Common-Legendary), `image_url` (Text)
    ...
    - **คอลัมน์สำคัญ**: `scene_slug`, `content_json` (บทพูด), `choices_json` (ทางเลือก), `action_type`, `next_scene_slug`
